...
As we saw, every functional call within \proc{Patch Update} takes in some meta data.
Some of this meta data needs to be calculated for every function call.
For example, \proc{Numerical Method} requires the coordinates for the center of the face it is processing.
These coordinates are calculated using the coordinates of the 2 cells that share this face.
However, \proc{Numerical Ingredients} such as Rusanov do not require this coordinate.
