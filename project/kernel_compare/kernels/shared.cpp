...
void kernels::shared::dim3::applySplit1DRiemannToPatch_Overlap1AoS3d (
  std::function<void(
    const double *__restrict__            QL,
    const double *__restrict__            QR,
  ...
  tarch::la::Vector<3, double> volumeH = kernels::shared::dim3::getVolumeSize (
      patchSize, numberOfVolumesPerAxisInPatch);
...
void kernels::shared::dim3::splitRusanov1d(
   std::function< void(
    const double * __restrict__ Q,
    const tarch::la::Vector<Dimensions,double>&  faceCentre,
    const tarch::la::Vector<Dimensions,double>&  volumeH,
...
tarch::la::Vector<3,double>  kernels::shared::dim3::getVolumeSize(
