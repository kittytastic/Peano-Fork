...
# different times
#
def plotMPIPhases(numberOfRanks,numberOfCores, inputFileName,fileName,upscaling):
  ...
  pylab.title( "trace of activities" )
  ax = pylab.gca()
  
  timeStampPattern = "([0-9]+\.?[0-9]*)"
  floatPattern = "([0-9]\.?[0-9]*)"
  
  beginIterationPattern              = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::beginIteration"
  leaveCentralElementPattern         = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::leaveCentralElementOfEnclosingSpacetree.*t_central-tree-traversal=\(" + floatPattern
  receiveDataFromWorkerPattern       = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::endToReceiveDataFromWorker.* for " + floatPattern
  receiveDataFromMasterPattern       = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::endToReceiveDataFromMaster.* for " + floatPattern
  releaseSynchronousHeapDataPattern  = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::endToReleaseSynchronousHeapData.*time=" + floatPattern
  releaseAsynchronousHeapDataPattern = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::endToPrepareAsynchronousHeapDataExchange.*time=" + floatPattern
  releaseJoinDataPattern             = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::endReleaseOfJoinData.*time=" + floatPattern
  releaseBoundaryDataPattern         = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::endReleaseOfBoundaryData.*time=" + floatPattern
  endBackgroundProcessing          = timeStampPattern + ".*rank:(\d+).*core:(\d+)*.*peano::performanceanalysis::DefaultAnalyser::endProcessingBackgroundJobs.*time=" + floatPattern

  def plotPhasesBar( rank, core, start, end, color):
    if end>start and (end-start)*upscaling>=1:
      rect = pylab.Rectangle([start*upscaling,(rank*numberOfCores+core-0.5)*upscaling],(end-start)*upscaling,upscaling,facecolor=color,edgecolor=color,alpha=Alpha)
      ax.add_patch(rect)
  ...
  try:
    ...
    for line in inputFile:
      ...
      m = re.search( endBackgroundProcessing, line )  
      if (m):
         timeStamp = float( m.group(1) )
         rank      = int( m.group(2) )
         core      = int( m.group(3) )   
	 duration  = float( m.group(4) )
         print "found match: ", timeStamp, rank, core, duration
	 plotPhasesBar(rank,core,timeStamp-duration,timeStamp,ColorReleaseBoundaryData)
