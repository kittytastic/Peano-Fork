...
tbb::task* tarch::multicore::jobs::internal::JobConsumerTask::execute() {
  ...
  if ( _bandwidthTasksAreProcessed.compare_and_swap(true,false) ) {
    hasProcessedJobs |= processJobs(internal::HighBandwidthTasksJobClassNumber,std::numeric_limits<int>::max(), -1, false);
    _bandwidthTasksAreProcessed = false;
  }

  hasProcessedJobs |= processJobs(internal::BackgroundTasksJobClassNumber,_maxJobs, -1, false);
