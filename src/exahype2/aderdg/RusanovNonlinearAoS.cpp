...
namespace exahype2 {
  namespace aderdg {
    ...
    #endif
    GPUCallableMethod void rusanovNonlinear_setBoundaryState_body_AoS(
      std::function< void(
    ...
    void rusanovNonlinear_loop_AoS(
      ...
      double * QBnd = nullptr;
      double * FBnd = nullptr;
      if ( leftCellIsOutside || rightCellIsOutside ) {
        double * FBnd = new double[nodesOnFace*strideF]{0.0};
      }
      ...
      for ( unsigned int scalarIndexFace = 0; scalarIndexFace < nodesOnFace; scalarIndexFace++ ) {
        if ( callFlux ) {
          rusanovNonlinear_riemannFlux_body_AoS(
            ...
            ( leftCellIsOutside )  ? FBnd : FLOut,
            ( rightCellIsOutside ) ? FBnd : FROut,
