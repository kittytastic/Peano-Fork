...
#ifdef UseVTK
vtkSmartPointer<vtkImageData> visualisation::output::PeanoWriter::toImageData(PeanoPatch *patch) {
...
vtkSmartPointer<vtkUnstructuredGrid> visualisation::output::PeanoWriter::toUnstructuredGrid(PeanoPatch *patch) {
...
int visualisation::output::PeanoWriter::xyzToIndex(int x, int y, int z, int dimensions[3]) {
	return x + y*(dimensions[0]+1) + z*(dimensions[0]+1)*(dimensions[1]+1);
}


std::string visualisation::output::PeanoWriter::writeFile(const std::vector<PeanoPatch*>& patches, const std::string& outputFileWithoutExtention) {
  #ifdef UseVTK
  if (!patches.empty()) {
    ...
    vtkSmartPointer<vtkUnstructuredGrid> outputGrid = visualisation::output::PeanoWriter::combine( patches );
...
#ifdef UseVTK
vtkSmartPointer<vtkUnstructuredGrid> visualisation::output::PeanoWriter::combine(const std::vector<PeanoPatch*>& patches){
...
vtkSmartPointer<vtkUnstructuredGrid> visualisation::output::PeanoWriter::combine(const std::vector<visualisation::input::PeanoTextPatchFileReader*>& readers) {
/*
	vtkSmartPointer<vtkAppendFilter> appendFilter = vtkSmartPointer<vtkAppendFilter>::New();
...
/*
PeanoPatch* visualisation::output::PeanoWriter::subSample(std::vector<PeanoReader*> &readers, int x, int y, int z) {
