...
class GenericCellKernel(ActionSet):
  """
  ...
  - The following C++ variables are defined:
    ...
    self.d[ "FACE_ACCESSOR" ]          = "fineGridFaces" + cell_data.name
  ...
  if ({GUARD}) {{
    ...
    double* faceData[Dimensions*2] = {{
      {FACE_ACCESSOR}(0).value,
      {FACE_ACCESSOR}(1).value,
      {FACE_ACCESSOR}(2).value,
      {FACE_ACCESSOR}(3).value
      #if Dimensions==3
      ,
      {FACE_ACCESSOR}(4).value,
      {FACE_ACCESSOR}(5).value,
      {FACE_ACCESSOR}(6).value,
      {FACE_ACCESSOR}(7).value
      #endif
    }};
    {IMPLEMENTATION}
  }}
