...
RECURSIVE SUBROUTINE PDENCP(BgradQ,Q,gradQin) 
  ...
  INTEGER :: i,j,k,l,m,n
  REAL(8) :: p,irho
  REAL(8) :: Qx(nVar),Qy(nVar),Qz(nVar) 
  REAL(8) :: k1,k2,k3,fff,ggg,e,c,mu,ds,xi,sk,bs,eta,itau
  REAL(8) :: fa,faa
  REAL(8) :: lapse,shift(3),gammaij(6),delta(3,3),vxb(3),vxb_contr(3),psi,qv_contr(3),qb_contr(3),bv_contr(3) 
  REAL(8) :: v2,vf(3),uem,b2,e2,gp,gm,vc(nVar),lf,w,ww,gamma1,rho,vf_cov(3),s_contr(3),w_ij,wim
  REAL(8) :: v_cov(3) 
  !
#ifdef VECTOR    
#ifdef AVX512 
...
#endif
  !
  REAL(8) :: QGRMHD(nVarGRMHD),gradQGRMHD(nVarGRMHD,d),BgradQGRMHD(nVarGRMHD)
...
RECURSIVE SUBROUTINE PDEFusedSrcNCP(Src_BgradQ,Q,gradQin)
  ...
  REAL(8) :: time=0.0D0
  INTEGER :: i,j,k,l,m,n
  REAL(8) :: p,irho,lam,mu 
  REAL(8) :: Qx(nVar),Qy(nVar),Qz(nVar),BgradQ(nVar),src(nVar),V(nVar)   
  REAL(8) :: k1,k2,k3,fff,ggg,e,c,ds,xi,sk,sknl,bs,dgup(3,3,3)
  REAL(8) :: fa,faa,u(3),ialpha 
  REAL(8) :: AQx(nVar),BQy(nVar),CQz(nVar),Smom(3),iphi2,ddm(3,3,3)   
  REAL(8) :: lapse,shift(3),gammaij(6),delta(3,3),bv(3),vxb(3),vxb_contr(3),psi,qv_contr(3),qb_contr(3),bv_contr(3) 
  REAL(8) :: v2,vf(3),uem,b2,e2,gp,gm,vc(nVar),lf,w,ww,gamma1,rho,vf_cov(3),s_contr(3),w_ij,wim 
  REAL(8) :: v_cov(3),v_contr(3),SijTF(3,3),phi2,EE 
  !
#ifdef VECTOR    
#ifdef AVX512 
  INTEGER, PARAMETER :: nVarGRMHD = 24                           ! The number of variables of the PDE system 
...
#endif
  !
  REAL(8) :: VGRMHD(nVarGRMHD), QGRMHD(nVarGRMHD), gradQGRMHD(nVarGRMHD,d), BgradQGRMHD(nVarGRMHD), eta, itau 
  REAL(8) :: SGRGPR(nVarGRGPR),QGRGPR(nVarGRGPR), VGRGPR(nVarGRGPR), gradQGRGPR(nVarGRGPR,d), BgradQGRGPR(nVarGRGPR)
  ! Q and their gradients
  ...
  REAL(8) :: Gtilde(3)
  REAL(8) :: Kex(3,3),Kmix(3,3),Kup(3,3)
  REAL(8) :: Christoffel(3,3,3),Christoffel_tilde(3,3,3),Christoffel_kind1(3,3,3) 
