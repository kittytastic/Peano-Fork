...
double getMinTimeStamp() {
  ...
  result = std::min( result, InstanceOfSWE.getMinTimeStamp() );
  return result;
}
...
double getMaxTimeStamp() {
  double result = 0.0;
  result = std::max( result, InstanceOfSWE.getMaxTimeStamp() );
  return result;
}
...
double getMinMeshSize() {
  double result = std::numeric_limits<double>::max();
  result = std::min( result, InstanceOfSWE.getMinMeshSize() );
  return result;
}
...
double getMaxMeshSize() {
  double result = 0.0;
  result = std::max( result, InstanceOfSWE.getMaxMeshSize() );
  return result;
}
...
double getMinTimeStepSize() {
  double result = std::numeric_limits<double>::max();
  result = std::min( result, InstanceOfSWE.getMinTimeStepSize() );
  return result;
}
...
double getMaxTimeStepSize() {
  double result = 0.0;
  result = std::max( result, InstanceOfSWE.getMaxTimeStepSize() );
  return result;
}


void startGridConstructionStep() {
  InstanceOfSWE.startGridConstructionStep();
...
void startGridInitialisationStep() {
  InstanceOfSWE.startGridInitialisationStep();
}
...
void startTimeStep(
  ...
  InstanceOfSWE.startTimeStep(minTimeStamp,maxTimeStamp,minTimeStepSize,maxTimeStepSize);
...
void startPlottingStep(
  ...
  InstanceOfSWE.startPlottingStep(minTimeStamp,maxTimeStamp,minTimeStepSize,maxTimeStepSize);
}


void finishTimeStep() {
  InstanceOfSWE.finishTimeStep();
...
void finishGridConstructionStep() {
  InstanceOfSWE.finishGridConstructionStep();
...
void finishGridInitialisationStep() {
  InstanceOfSWE.finishGridInitialisationStep();
}


void finishPlottingStep() {
  InstanceOfSWE.finishPlottingStep();
}
