...
// here
//  
#ifndef _INCLUDE_EXAMPLES_EXAHYPE2_EULER_CELLDATA_EULERCELLSEMAPHORELABEL_
#define _INCLUDE_EXAMPLES_EXAHYPE2_EULER_CELLDATA_EULERCELLSEMAPHORELABEL_
...
namespace celldata{
  ...
  struct EulerCellSemaphoreLabel;
}
}
}
}
...
struct examples::exahype2::euler::celldata::EulerCellSemaphoreLabel {
  public:


    EulerCellSemaphoreLabel() {}
    EulerCellSemaphoreLabel(tarch::la::Vector<Dimensions,double>  __debugX, tarch::la::Vector<Dimensions,double>  __debugH, int  __SemaphoreNumber);
     ...
     * blocking MPI_Send. 
     */
    static void send(const examples::exahype2::euler::celldata::EulerCellSemaphoreLabel& buffer, int destination, int tag, MPI_Comm communicator );
    static void receive(examples::exahype2::euler::celldata::EulerCellSemaphoreLabel& buffer, int source, int tag, MPI_Comm communicator );
     ...
     */
    static void send(const examples::exahype2::euler::celldata::EulerCellSemaphoreLabel& buffer, int destination, int tag, std::function<void()> waitFunctor, MPI_Comm communicator );
    static void receive(examples::exahype2::euler::celldata::EulerCellSemaphoreLabel& buffer, int source, int tag, std::function<void()> waitFunctor, MPI_Comm communicator );
    #endif


#ifdef Parallel
    static void sendAndPollDanglingMessages(const examples::exahype2::euler::celldata::EulerCellSemaphoreLabel& message, int destination, int tag );
    static void receiveAndPollDanglingMessages(examples::exahype2::euler::celldata::EulerCellSemaphoreLabel& message, int source, int tag );
#endif
    
    void merge(const examples::exahype2::euler::celldata::EulerCellSemaphoreLabel& neighbour, const peano4::datamanagement::CellMarker& marker);
