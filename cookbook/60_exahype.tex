...
\begin{itemize}
  ...
  \item Study the header/implementation file pair \texttt{toolbox/blockstructured/Interpolation}. Different interpolation/restriction variants differ in their suffix. 
  The routines \linebreak \texttt{piecewise\_constant} are those used by default.
  \item Make your solver (in Python) use another scheme. I present a variant here where you want to use linear interpolation:
  \begin{code}
...
self._action_set_couple_resolution_transitions_and_handle_dynamic_mesh_refinement = DynamicAMR( 
  ...
  interpolation_scheme="linear",  # by default set to "piecewise_constant"
  restriction_scheme="averaging"   # this is the default
 )
  \end{code}
  You might need an
  \begin{code}
...
\noindent
\texttt{targetPatch} is a double pointer to the volume that has been interpolated before.
