...
So my Peano master points to version 3.
For the present document, it is important that you manually switch to the
branch \texttt{p4}.
...
Change into the project's directory and type in\footnote{This step will fail if
you work against the git checkout where I don't store the configure script's
outcomes. If you work against the git version, please type in
\texttt{libtoolize; aclocal; autoconf; autoheader; cp src/config.h.in .;
automake --add-missing} before you continue.}
\begin{code}
./configure --help
\end{code}
