...
\begin{enumerate}
  ...
  \item We augment the state: from hereon, the state can co-decide whether and
    ...
    void endedGridConstructionIteration();
...
void storageschemes::State::endedGridConstructionIteration() {
  ...
  static bool traversalsThatWouldHaveBeenAbleToIncreaseLevelInLastSweepInLastSweep 
    = false;
  ...
  else if ( nodePoolHasGivenOutRankSizeLastQuery && isGridStationary()
    && !traversalsThatWouldHaveBeenAbleToIncreaseLevelInLastSweepInLastSweep
  ) {
    traversalsThatWouldHaveBeenAbleToIncreaseLevelInLastSweepInLastSweep = true;
    changedCellState();
  }
  else if ( nodePoolHasGivenOutRankSizeLastQuery && isGridStationary() ) {
    _stateData.setMaxRefinementLevelAllowed(
      _stateData.getMaxRefinementLevelAllowed()+1);
    traversalsThatWouldHaveBeenAbleToIncreaseLevelInLastSweepInLastSweep = false;
  }
}
  \end{code}
...
{ 
 ...
 \item (Optional) It did pay off in my applications to alter grid
 construction iterations making them use the more sophisticated state, too: 
  \begin{code}
  int sweepsWithBalancedGrid = 0;
  while ( sweepsWithBalancedGrid<3 ) {
    repository.iterate();
    sweepsWithBalancedGrid = repository.getState().isGridBalanced() ? 
      sweepsWithBalancedGrid+1 : 0;
    if (repository.getState().mayRefine(true,std::numeric_limits<int>::max())) {
      sweepsWithBalancedGrid = 3;
    }
    repository.getState().endedLoadBalancingIteration();
  }
  \end{code}
  Here, we terminate the construction immediately one we bump into the enforce
  refinement: We know that now the grid will be built up completely, so we wait
  one more grid sweep (therefore
  we have to insert the if statement just before
  \texttt{endedLoadBalancingIteration}) until all refinements have passed
  through, end then we terminate the loop. There's no need to wait for the grid
  to become balanced.
\end{enumerate}
%\end{remark}
